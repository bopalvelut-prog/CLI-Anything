...
from cli_anything.archfont.core.session import Session
...
@handle_error
def run_cmd(args):
    """Run arbitrary archfont command."""
    import subprocess
    result=subprocess.run(["archfont"]+list(args),capture_output=True,text=True,timeout=300)
    output({"status":"success" if result.returncode==0 else "error","stdout":result.stdout,"stderr":result.stderr})
@cli.command()
@handle_error
def repl():
    from cli_anything.archfont.utils.repl_skin import ReplSkin
    global _repl_mode;_repl_mode=True
    skin=ReplSkin("archfont",version="1.0.0");skin.print_banner()
