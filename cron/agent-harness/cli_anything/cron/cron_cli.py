...
from cli_anything.cron.core.session import Session
...
@handle_error
def repl():
    from cli_anything.cron.utils.repl_skin import ReplSkin
    global _repl_mode;_repl_mode=True
    skin=ReplSkin("cron",version="1.0.0");skin.print_banner()
    ...
    while True:
        try:
            ...
            if line.lower()=="help":skin.help({"list":"List cron jobs","add --entry ENTRY":"Add entry","remove --line N":"Remove line N","edit":"Edit crontab","validate --entry ENTRY":"Validate entry"});continue
...
@handle_error
def list():
    from cli_anything.cron.core.scheduler import list_jobs
    result = list_jobs()
    output(result, "Cron jobs")
@cli.command()
@click.option("--entry","entry",required=True,help="Cron entry")
@handle_error
def add(entry):
    from cli_anything.cron.core.scheduler import add_job
    result = add_job(entry)
    output(result, "Added")
@cli.command()
@click.option("--line","line_num",required=True,type=int,help="Line number to remove")
@handle_error
def remove(line_num):
    from cli_anything.cron.core.scheduler import remove_job
    result = remove_job(line_num)
    output(result, "Removed")
@cli.command()
@handle_error
def edit():
    from cli_anything.cron.core.scheduler import edit_crontab
    result = edit_crontab()
    output(result, "Edited")
@cli.command()
@click.option("--entry","entry",required=True,help="Cron entry to validate")
@handle_error
def validate(entry):
    from cli_anything.cron.core.scheduler import validate_entry
