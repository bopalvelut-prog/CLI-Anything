...
from cli_anything.ambilist.core.session import Session
...
@handle_error
def run_cmd(args):
    """Run arbitrary ambilist command."""
    import subprocess
    result=subprocess.run(["ambilist"]+list(args),capture_output=True,text=True,timeout=300)
    output({"status":"success" if result.returncode==0 else "error","stdout":result.stdout,"stderr":result.stderr})
@cli.command()
@handle_error
def repl():
    from cli_anything.ambilist.utils.repl_skin import ReplSkin
    global _repl_mode;_repl_mode=True
    skin=ReplSkin("ambilist",version="1.0.0");skin.print_banner()
