#!/usr/bin/env python3
import sys,os,json,shlex,click
sys.path.insert(0,os.path.dirname(os.path.dirname(os.path.abspath(__file__))))
from cli_anything.galaxy.core.session import Session
_session=None;_json_output=False;_repl_mode=False
def get_session():
    global _session
    if _session is None:_session=Session()
    return _session
def output(data,message=""):
    if _json_output:click.echo(json.dumps(data,indent=2,default=str))
    else:
        if message:click.echo(message)
        if isinstance(data,dict):
            for k,v in data.items():click.echo(f"  {k}: {v}")
        elif isinstance(data,list):
            for i,item in enumerate(data):
                if isinstance(item,dict):
                    click.echo(f"  [{i}]")
                    for k,v in item.items():click.echo(f"    {k}: {v}")
                else:click.echo(f"  - {item}")
        else:click.echo(str(data))
def handle_error(func):
    def wrapper(*args,**kwargs):
        try:return func(*args,**kwargs)
        except Exception as e:
            if _json_output:click.echo(json.dumps({"error":str(e),"type":type(e).__name__}))
            else:click.echo(f"Error: {e}",err=True)
            if not _repl_mode:sys.exit(1)
    wrapper.__name__=func.__name__;wrapper.__doc__=func.__doc__;return wrapper
@click.group(invoke_without_command=True)
@click.option("--json","use_json",is_flag=True)
@click.pass_context
def cli(ctx,use_json):
    global _json_output;_json_output=use_json
    if ctx.invoked_subcommand is None:ctx.invoke(repl)
@cli.command()
@handle_error
def repl():
    from cli_anything.galaxy.utils.repl_skin import ReplSkin
    global _repl_mode;_repl_mode=True
    skin=ReplSkin("galaxy",version="1.0.0");skin.print_banner()
    pt_session=skin.create_prompt_session()
    while True:
        try:
            line=skin.get_input(pt_session)
            if not line:continue
            if line.lower() in ("quit","exit","q"):skin.print_goodbye();break
            if line.lower()=="help":skin.help({"search --query Q":"Search roles","info --role R":"Role info","install --role R":"Install role","init --role-name N":"Init role","list":"List installed","remove --role R":"Remove role","collection install --name N":"Install collection"});continue
            try:args=shlex.split(line)
            except:args=line.split()
            try:cli.main(args,standalone_mode=False)
            except SystemExit:pass
            except click.exceptions.UsageError as e:skin.warning(f"Usage error: {e}")
            except Exception as e:skin.error(f"{e}")
        except (EOFError,KeyboardInterrupt):skin.print_goodbye();break
    _repl_mode=False
@cli.command()
@click.option("--query","query",required=True,help="Search query")
@handle_error
def search(query):
    from cli_anything.galaxy.core.roles import search_roles
    result = search_roles(query)
    output(result, "Search results")
@cli.command()
@click.option("--role","role",required=True,help="Role name")
@handle_error
def info(role):
    from cli_anything.galaxy.core.roles import role_info
    result = role_info(role)
    output(result)
@cli.command()
@click.option("--role","role",required=True,help="Role name")
@handle_error
def install(role):
    from cli_anything.galaxy.core.roles import install_role
    result = install_role(role)
    output(result, f"Installed {role}")
@cli.command()
@click.option("--role-name","role_name",required=True,help="Role name")
@handle_error
def init(role_name):
    from cli_anything.galaxy.core.roles import init_role
    result = init_role(role_name)
    output(result, f"Initialized {role_name}")
@cli.command()
@handle_error
def list():
    from cli_anything.galaxy.core.roles import list_roles
    result = list_roles()
    output(result, "Installed roles")
@cli.command()
@click.option("--role","role",required=True,help="Role name")
@handle_error
def remove(role):
    from cli_anything.galaxy.core.roles import remove_role
    result = remove_role(role)
    output(result, f"Removed {role}")
@cli.group("collection")
@handle_error
def collection():
    pass
@collection.command()
@click.option("--name","name",required=True,help="Collection name")
@handle_error
def install(name):
    from cli_anything.galaxy.core.roles import install_collection
    result = install_collection(name)
    output(result, f"Installed collection {name}")
def main():cli()
if __name__=="__main__":main()
