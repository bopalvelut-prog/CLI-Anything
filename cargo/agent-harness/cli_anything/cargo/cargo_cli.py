...
from cli_anything.cargo.core.session import Session
...
def run_cmd(args):
    ...
    import subprocess
    result=subprocess.run(["cargo"]+list(args),capture_output=True,text=True,timeout=300)
    output({"status":"success" if result.returncode==0 else "error","stdout":result.stdout,"stderr":result.stderr})

@cli.command()
@handle_error
def repl():
    from cli_anything.cargo.utils.repl_skin import ReplSkin
    global _repl_mode;_repl_mode=True
    skin=ReplSkin("cargo",version="1.0.0");skin.print_banner()
