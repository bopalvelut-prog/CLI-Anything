...
from cli_anything.archiaas.core.session import Session
...
@handle_error
def run_cmd(args):
    """Run arbitrary archiaas command."""
    import subprocess
    result=subprocess.run(["archiaas"]+list(args),capture_output=True,text=True,timeout=300)
    output({"status":"success" if result.returncode==0 else "error","stdout":result.stdout,"stderr":result.stderr})
@cli.command()
@handle_error
def repl():
    from cli_anything.archiaas.utils.repl_skin import ReplSkin
    global _repl_mode;_repl_mode=True
    skin=ReplSkin("archiaas",version="1.0.0");skin.print_banner()
