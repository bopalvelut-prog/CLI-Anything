...
from cli_anything.arch_skin.core.session import Session
...
@handle_error
def run_cmd(args):
    """Run arbitrary arch_skin command."""
    import subprocess
    result=subprocess.run(["arch_skin"]+list(args),capture_output=True,text=True,timeout=300)
    output({"status":"success" if result.returncode==0 else "error","stdout":result.stdout,"stderr":result.stderr})
@cli.command()
@handle_error
def repl():
    from cli_anything.arch_skin.utils.repl_skin import ReplSkin
    global _repl_mode;_repl_mode=True
    skin=ReplSkin("arch_skin",version="1.0.0");skin.print_banner()
