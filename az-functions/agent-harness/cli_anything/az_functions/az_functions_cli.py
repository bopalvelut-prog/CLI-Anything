...
from cli_anything.az_functions.core.session import Session
...
@handle_error
def run_cmd(args):
    """Run arbitrary az_functions command."""
    import subprocess
    result=subprocess.run(["az_functions"]+list(args),capture_output=True,text=True,timeout=300)
    output({"status":"success" if result.returncode==0 else "error","stdout":result.stdout,"stderr":result.stderr})
@cli.command()
@handle_error
def repl():
    from cli_anything.az_functions.utils.repl_skin import ReplSkin
    global _repl_mode;_repl_mode=True
    skin=ReplSkin("az_functions",version="1.0.0");skin.print_banner()
