...
from cli_anything.anteread.core.session import Session
...
@handle_error
def run_cmd(args):
    """Run arbitrary anteread command."""
    import subprocess
    result=subprocess.run(["anteread"]+list(args),capture_output=True,text=True,timeout=300)
    output({"status":"success" if result.returncode==0 else "error","stdout":result.stdout,"stderr":result.stderr})
@cli.command()
@handle_error
def repl():
    from cli_anything.anteread.utils.repl_skin import ReplSkin
    global _repl_mode;_repl_mode=True
    skin=ReplSkin("anteread",version="1.0.0");skin.print_banner()
