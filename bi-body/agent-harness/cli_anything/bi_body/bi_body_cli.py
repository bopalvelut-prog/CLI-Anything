...
from cli_anything.bi_body.core.session import Session
...
@handle_error
def run_cmd(args):
    """Run arbitrary bi_body command."""
    import subprocess
    result=subprocess.run(["bi_body"]+list(args),capture_output=True,text=True,timeout=300)
    output({"status":"success" if result.returncode==0 else "error","stdout":result.stdout,"stderr":result.stderr})
@cli.command()
@handle_error
def repl():
    from cli_anything.bi_body.utils.repl_skin import ReplSkin
    global _repl_mode;_repl_mode=True
    skin=ReplSkin("bi_body",version="1.0.0");skin.print_banner()
