...
from cli_anything.anti_map.core.session import Session
...
@handle_error
def run_cmd(args):
    """Run arbitrary anti_map command."""
    import subprocess
    result=subprocess.run(["anti_map"]+list(args),capture_output=True,text=True,timeout=300)
    output({"status":"success" if result.returncode==0 else "error","stdout":result.stdout,"stderr":result.stderr})
@cli.command()
@handle_error
def repl():
    from cli_anything.anti_map.utils.repl_skin import ReplSkin
    global _repl_mode;_repl_mode=True
    skin=ReplSkin("anti_map",version="1.0.0");skin.print_banner()
