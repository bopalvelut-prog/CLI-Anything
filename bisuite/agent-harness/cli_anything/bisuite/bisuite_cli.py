...
from cli_anything.bisuite.core.session import Session
...
@handle_error
def run_cmd(args):
    """Run arbitrary bisuite command."""
    import subprocess
    result=subprocess.run(["bisuite"]+list(args),capture_output=True,text=True,timeout=300)
    output({"status":"success" if result.returncode==0 else "error","stdout":result.stdout,"stderr":result.stderr})
@cli.command()
@handle_error
def repl():
    from cli_anything.bisuite.utils.repl_skin import ReplSkin
    global _repl_mode;_repl_mode=True
    skin=ReplSkin("bisuite",version="1.0.0");skin.print_banner()
