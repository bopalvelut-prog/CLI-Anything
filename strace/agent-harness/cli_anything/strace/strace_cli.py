...
@handle_error
def repl():
    from cli_anything.strace.utils.repl_skin import ReplSkin
    global _repl_mode;_repl_mode=True
    skin=ReplSkin("strace",version="1.0.0");skin.print_banner()
    ...
    while True:
        try:
            ...
            if line.lower()=="help":skin.help({"trace":"Trace a binary","trace --follow":"Trace with follow forks","trace --filter SYSCALL":"Filter syscalls","trace --output FILE":"Output to file","trace --summary":"Summary mode","trace --count":"Count mode"});continue
...
@click.option("--follow",is_flag=True,help="Follow forks")
@click.option("--filter","sys_filter",help="Filter syscall pattern")
@click.option("--output","outfile",help="Write trace to file")
@click.option("--summary",is_flag=True,help="Summary mode")
@click.option("--count",is_flag=True,help="Count mode")
@handle_error
def trace(binary,follow,sys_filter,outfile,summary,count):
    from cli_anything.strace.core.trace import run_trace
    result = run_trace(binary, follow=follow, sys_filter=sys_filter, outfile=outfile, summary=summary, count=count)
    output(result, "Trace complete")
def main():cli()
