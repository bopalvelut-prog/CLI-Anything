...
from cli_anything.antiframe.core.session import Session
...
@handle_error
def run_cmd(args):
    """Run arbitrary antiframe command."""
    import subprocess
    result=subprocess.run(["antiframe"]+list(args),capture_output=True,text=True,timeout=300)
    output({"status":"success" if result.returncode==0 else "error","stdout":result.stdout,"stderr":result.stderr})
@cli.command()
@handle_error
def repl():
    from cli_anything.antiframe.utils.repl_skin import ReplSkin
    global _repl_mode;_repl_mode=True
    skin=ReplSkin("antiframe",version="1.0.0");skin.print_banner()
