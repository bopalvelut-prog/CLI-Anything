...
from cli_anything.aws_polly.core.session import Session
...
@handle_error
def run_cmd(args):
    """Run arbitrary aws_polly command."""
    import subprocess
    result=subprocess.run(["aws_polly"]+list(args),capture_output=True,text=True,timeout=300)
    output({"status":"success" if result.returncode==0 else "error","stdout":result.stdout,"stderr":result.stderr})
@cli.command()
@handle_error
def repl():
    from cli_anything.aws_polly.utils.repl_skin import ReplSkin
    global _repl_mode;_repl_mode=True
    skin=ReplSkin("aws_polly",version="1.0.0");skin.print_banner()
