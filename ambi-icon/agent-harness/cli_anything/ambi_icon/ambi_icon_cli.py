...
from cli_anything.ambi_icon.core.session import Session
...
@handle_error
def run_cmd(args):
    """Run arbitrary ambi_icon command."""
    import subprocess
    result=subprocess.run(["ambi_icon"]+list(args),capture_output=True,text=True,timeout=300)
    output({"status":"success" if result.returncode==0 else "error","stdout":result.stdout,"stderr":result.stderr})
@cli.command()
@handle_error
def repl():
    from cli_anything.ambi_icon.utils.repl_skin import ReplSkin
    global _repl_mode;_repl_mode=True
    skin=ReplSkin("ambi_icon",version="1.0.0");skin.print_banner()
